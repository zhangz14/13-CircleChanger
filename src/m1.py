...
class CircleChanger(object):
    ...
    def get_distance_from(self, point):
        ...
        ################################################################
        d = ((self.circle.center.x - point.x) ** 2 + (self.circle.center.y - point.y) ** 2) ** (1 / 2)
        return d
    ...
    def swell_or_shrink_once(self, amount_to_swell_or_shrink):
        ...
        elif self.circle.radius + amount_to_swell_or_shrink < 1:
    ...
    def swallow(self, other_circle_changer):
        ...
        #   NO CREDIT if you use the distance formula here.
        ################################################################
        new_center = self.circle.center.halfway_to(other_circle_changer.circle.center)
        new_radius = self.get_distance_from(other_circle_changer.circle.center) / 2
        self.circle = rg.Circle(new_center, new_radius)
    ...
    def change_to_next_color_in_tuple(self):
        ...
        if self.count == 1:
            self.change_color(0)
        elif self.count > len(self.colors):
            self.count = 1
            self.change_color(0)
        else:
            self.change_color(self.count - 1)
